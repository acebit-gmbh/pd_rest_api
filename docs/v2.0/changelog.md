...
- **`GET /databases/{db}/categories`** returns the category list a database carries -- the names the Windows client offers in the picker of its entry and folder editors -- as plain strings, sorted and without duplicates: `{"data": ["Banking", "Internet", "Social"], "total": 3}`. A database with no category answers `{"data": [], "total": 0}`. The route is **not paginated**: it reads no `offset` and no `limit`, the envelope holds `data` and `total` only, and `data` is always the whole list, because an entry editor needs all of it and it is a handful of short strings. Whoever may open the database may read it, in `client` and `admin` sessions alike and on a mirror server; this is metadata about the database, like the icon list, and it names no entry. A database the caller cannot reach answers `404`. Only `GET`: any other method answers `405` with `Allow: GET`, and a further path segment answers `404`.
- **A category is created by saving an entry or a folder.** `POST` and `PATCH` on an entry **and** on a folder add the item's category to that list when the list does not already hold it - on a `PATCH` that is the category the item carries after the write, whether or not the body sent one, so that every editor offers it from then on -- the same thing a save in the Windows client does. The match ignores letter case: sending `"banking"` where the list holds `"Banking"` adds nothing and changes nothing, the list keeps its own spelling, and the item keeps the spelling it was sent. The name is trimmed before it is added, and an empty or blank `category` adds nothing. **Nothing removes a name:** deleting the last entry or folder of a category leaves the category in the list. There is no route that edits the list.
- **No capability object.** Unlike the recycle bin and database icons, this has no marker on the database object: the route itself is the test. A server that does not have it answers a plain `404` -- which is also the answer for a database that does not exist or is not accessible, so call it on a database that has already answered and keep the result for the session.
