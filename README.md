...
- **Long-lived API tokens** -- Tokens for automation with a fixed expiry date 1 to 730 days ahead (180 by default; the token stops working at 00:00 on that date), generated by a Super Administrator in the Server Manager for any user account, in client or admin scope, and revoked or deleted there; they do not lapse on inactivity and are not revoked by `/auth/logout`
...
- **Pagination on list endpoints** -- `?offset=...&limit=...` with a standardized `{data, total, offset, limit}` envelope on every JSON list, the OIDC provider list included; the one exception is the audit export (`GET /admin/audit`), which returns NDJSON (`application/x-ndjson`) and takes `limit` but no `offset`
...
- **Case-sensitive routes** -- v2.0 route names are matched case-sensitively, per RFC 3986 (`/v2.0/ME` answers `404`); identifiers in the path are not case-sensitive, and the `permissions`, `children` and `move` sub-resources of databases and folders are still matched case-insensitively. v1.0, on 19.x servers, is case-insensitive
